...
export abstract class BridgeEvent extends BaseBridgeEvent {
  setHandler(arg0: ((res: AsyncResult<boolean>) => void) | Handler<AsyncResult<boolean>>) : Future<boolean>;
  ...
  compose<U>(handler: ((res: boolean) => void) | Handler<boolean>, next: Future<U>) : Future<U>;

  compose<U>(mapper: (arg: boolean) => Future<U>) : Future<U>;

  map<U>(mapper: (arg: boolean) => U) : Future<U>;

  completer() : ((res: AsyncResult<boolean>) => void) | Handler<AsyncResult<boolean>>;

  recover(mapper: (arg: Error) => Future<boolean>) : Future<boolean>;

  otherwise(mapper: (arg: Error) => boolean) : Future<boolean>;
...
export abstract class ErrorHandler implements Handler<RoutingContext> {
   ...
   * 
   */
  static readonly DEFAULT_DISPLAY_EXCEPTION_DETAILS : boolean;
...
export abstract class SockJSHandler implements Handler<RoutingContext> {
   ...
   * 
   */
  socketHandler(handler: ((res: SockJSSocket) => void) | Handler<SockJSSocket>) : SockJSHandler;
   ...
   * 
   */
  bridge(bridgeOptions: BridgeOptions, bridgeEventHandler: ((res: BridgeEvent) => void) | Handler<BridgeEvent>) : SockJSHandler;
}
...
import { TemplateEngine } from '@vertx/web-common';

export abstract class TemplateEngine extends TemplateEngine {
  /**
   * Render the template
   * @param context  the routing context
